...
| Primary focus | query helpers, typed data access, shared request patterns |
...
- This repo currently publishes `@platform/query` as the legacy compatibility package id while the migration to `@gutu/query` is completed.
